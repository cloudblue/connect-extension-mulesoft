...
public class ListChunkFileOperation extends BaseListOperation {
    @MediaType(value = ANY, strict = false)
    @DisplayName("List Usage Chunk Files")
    public List<CBCUsageChunkFile> listUsageChunkFiles(
            @Connection CBCConnection connection
    ) throws CBCException {

        Client.Q q = connection
                .newQ(new TypeReference<ArrayList<CBCUsageChunkFile>>() {})
                .collection(USAGE)
                .collection(CHUNKS);

        resolve(q);

        return (List<CBCUsageChunkFile>) q.get();
    }
}
