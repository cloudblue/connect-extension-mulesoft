...
public class GetResourceOperation {
    @Parameter
    ...
    public Result<InputStream, CBCResponseAttributes> getResource(
        ...
        String id = (String) getResourceParameter.get(metadata.getId().getField());

        CBCConnection.Q q = connection.newQ();

        if (metadata.isSubCollection()) {
            String parentId = (String) getResourceParameter.get(metadata.getParentId().getField());

            q.collection(metadata.getParentCollection(), parentId);
        }
        return q.collection(metadata.getCollection(), id).get();
    }
}
